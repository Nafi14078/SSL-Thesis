...
train_dataset = BratsSSLDataset(
    ...
    task="tans"   # Use TANS noise for training
)

val_dataset = BratsSSLDataset(
    data_dir=DATA_DIR,
    file_list_path=VAL_SPLIT,
    task="tans"   # Use TANS noise for validation
)
...
print("🚀 Starting Denoising Training on GPU...\n")
...
plt.xlabel("Epoch")
plt.ylabel("MSE Loss")
plt.title("Denoising tans Training Curve")
plt.legend()
plt.grid(True)
